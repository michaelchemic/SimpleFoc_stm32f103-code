...
uint8_t USAR2_RxData;
uint8_t USAR3_RxData;
uint8_t USAR_RxFlag;
...
uint8_t USAR2_GetRxData(void) // 得到接收到的数据的值
{
    return USAR2_RxData;
}

uint8_t USAR3_GetRxData(void) // 得到接收到的数据的值
{
    return USAR3_RxData;
}
...
{
    if (USART_GetITStatus(USART3, USART_IT_RXNE) == SET) // 如果串口接收到数据
    {
        USAR3_RxData = USART_ReceiveData(USART3); // 接收数据赋予全局变量USAR_RxData
        USAR_RxFlag  = 1;
...
void USART2_IRQHandler(void)
{
    ...
    {
        USAR2_RxData = USART_ReceiveData(USART2); // 接收数据赋予全局变量USAR_RxData
        USAR_RxFlag  = 1;
        USART_ClearITPendingBit(USART2, USART_IT_RXNE);

        if (USAR2_RxData == 0xAA) {
            rx_index              = 0;            // 重新开始接收数据
            rx_buffer[rx_index++] = USAR2_RxData; // 存储帧头
        } else if (rx_index != 0) {
            rx_buffer[rx_index++] = USAR2_RxData; // 存储数据

            if (USAR2_RxData == 0x55) { // 若接收到帧尾
